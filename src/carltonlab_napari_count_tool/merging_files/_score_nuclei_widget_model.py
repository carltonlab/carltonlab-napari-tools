...
def _load_scored_nuclei_foci_count(
    ...
    points_file_name = (
        f"{region_name}_{sbs_name}{SCORED_NUCLEI_POINTS_FILE_NAME_EXTENSION}"
    )
    points_file_path = os.path.join(scored_nuclei_dir, points_file_name)
    zero_points_file_name = f"{region_name}_{sbs_name}_zero_points.txt"
...
def generate_scored_nuclei_foci_summary(
    ...
    for region_index in range(len(nuclei_count)):
        ...
        region_sbs_counts.append(sbs_count)
        max_sbs_count = max(max_sbs_count, sbs_count)

    foci_counts: dict[str, list[int | None]] = {
        region_name: [None] * max_sbs_count for region_name in region_names
    }
    for region_name, sbs_count in zip(
        region_names, region_sbs_counts, strict=True
    ):
        for sbs_index in range(1, sbs_count + 1):
            count = _load_scored_nuclei_foci_count(
                scored_nuclei_dir, region_name, sbs_index
            )
            foci_counts[region_name][sbs_index - 1] = count

    output_df = pd.DataFrame(
        foci_counts, index=list(range(1, max_sbs_count + 1))
    )
    output_df.index.name = "sbs"
    if output_csv_path is None:
        output_csv_path = os.path.join(
